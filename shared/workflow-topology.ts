...
function validateWorkflow(workflow: WorkflowLike): WorkflowValidationIssue[] {
  ...
  for (const node of workflow.nodes) {
    if (node.fallbackTo) {
      ...
      connected.add(node.fallbackTo);
    }
  }
  for (const node of workflow.nodes) {
    if (workflow.nodes.length > 1 && !connected.has(node.id)) {
      issues.push({ type: 'warning', nodeId: node.id, message: `Node ${node.id} is disconnected` });
    }
  }
...
export function analyzeWorkflowTopology(rawWorkflow: any, options: AnalyzeWorkflowOptions = {}): WorkflowTopologyAnalysis {
  ...
  const validationIssues = options.validationIssues ?? validateWorkflow(workflow);
  ...
  const disconnectedNodes = validationIssues
    .filter(issue => issue.type === 'warning' && issue.message.includes('disconnected') && issue.nodeId)
