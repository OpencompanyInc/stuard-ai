        ...
        try {
          ...
          if (msg.type === 'handshake') {
            ...
            if (evt.event === 'routing') {
              ...
              const emitSyntheticTool = (toolName: string, syntheticId: string, status: ToolCall['status'], args?: any, result?: any) => {
                updateStreamingTab(t => {
                  ...
                  const nextCall: ToolCall = existing
                    ? { ...existing, tool: toolName, status, args: typeof args !== 'undefined' ? args : existing.args, result: typeof result !== 'undefined' ? result : existing.result }
                    : { id: syntheticId, tool: toolName, status, args, result, timestamp: Date.now() };
              ...
              if (HIDDEN_WRAPPER_TOOL_NAMES.has(tool)) {
                ...
                if (stepTool) {
                  ...
                  if (normalizedStatus === 'step_started') {
                    ...
                    emitSyntheticTool(stepTool, syntheticId, st, undefined, normalizedStatus === 'step_completed' ? evt.data?.result : { ok: false, error: evt.data?.error || 'failed' });
