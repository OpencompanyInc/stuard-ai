...
export function createWindow() {
  ...
  win.on('blur', () => registerMoveShortcuts());
...
export function openWorkflowsWindow() {
  ...
  if (isDev) {
    d.loadURL(getRendererUrl("workflows"));
  } else {
    const candidates = [
...
export function setOverlayMode(mode: OverlayMode) {
  ...
  if (mode === "sidebar") {
    const { workArea } = screen.getPrimaryDisplay();
    // Sidebar: full height on right, thicker width for better UX
    const w = 520;
    const h = workArea.height;
    win?.setBounds({ x: workArea.x + workArea.width - w, y: workArea.y, width: w, height: h });
    setOverlaySize(w, h, false); // Sidebar handles its own positioning
    // Notify renderer of mode change
    try { win?.webContents.send('overlay:modeChanged', { mode, width: w, height: h, prevMode }); } catch { }
    return;
  }
