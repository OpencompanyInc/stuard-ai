...
interface ActionDescriptor {
  ...
  /** Icon for the mark when the tool has no integration brand (e.g. voice). */
  icon?: LucideIcon;
}
...
function describeSuccess(
  ...
  if (
    ...
    const included =
      num(result?.attachmentsIncluded) ??
      num(result?.attachments_included) ??
      0;
    const chips: string[] = [];
    if (included > 0)
      chips.push(`${included} attachment${included === 1 ? "" : "s"}`);
  ...
  if (name === "calendar_create_event" || name === "calendar_quick_add") {
    const meta: React.ReactNode[] = [];
    const summary =
      str(args?.summary) || str(args?.text) || str(result?.event?.summary);
  ...
  if (
    ...
    name === "drive_create_from_text"
  ) {
    return {
      title: "Saved to Drive",
      meta: metaOne(str(result?.file?.name) || str(args?.name)),
      link: linkFrom(result),
    };
  }
  if (name === "drive_download_file" || name === "drive_export_file") {
    return {
      ...
      meta: metaOne(str(result?.path) || str(args?.path)),
    };
  }
  if (name === "drive_share_file") {
    const who =
      str(result?.permission?.emailAddress) || str(args?.emailAddress);
    const role = str(result?.permission?.role) || str(args?.role);
    return {
      title: "Shared from Drive",
      meta: metaOne(who),
      chips: role ? [role] : undefined,
    };
  }
  ...
  if (name === "drive_trash_file")
    return { title: "Moved to Drive trash", meta: metaOne(str(result?.name)) };
  if (name === "drive_move_file")
    return { title: "Moved in Drive", meta: metaOne(str(result?.file?.name)) };
  if (name === "drive_rename_file" || name === "drive_copy_file") {
    return {
      title:
        name === "drive_copy_file" ? "Copied in Drive" : "Renamed in Drive",
      meta: metaOne(str(result?.file?.name) || str(args?.name)),
    };
  }
  ...
  if (name === "docs_create" || name === "docs_create_document") {
    return {
      title: "Doc created",
      meta: metaOne(str(args?.title) || str(result?.title)),
      link: linkFrom(result),
    };
  }
  ...
  if (name === "tasks_complete")
    return { title: "Task completed", meta: metaOne(str(args?.title)) };
  if (name === "tasks_delete") return { title: "Task deleted" };
  if (name.startsWith("tasks_") && looksLikeAction(name))
    return { title: "Task updated", meta: metaOne(str(args?.title)) };
  ...
  if (name === "x_repost" || name === "x_retweet")
    return { title: "Reposted on X", link: linkFrom(result) };
  if (
    name === "x" ||
    name === "x_post" ||
    name === "x_create_post" ||
    ...
    return {
      title: "Posted to X",
      body: str(args?.text) || str(args?.status) || undefined,
      link: linkFrom(result),
    };
  }
  ...
  if (
    ...
    return {
      ...
      meta: channel
        ? metaOne(channel.startsWith("#") ? channel : `#${channel}`)
        : undefined,
      body: str(args?.text) || undefined,
      link: linkFrom(result),
    };
  ...
  if (name === "notion_create_page") {
    return {
      ...
      meta: metaOne(str(args?.title) || titleFromNotion(args)),
      link: linkFrom(result),
    };
  }
  if (name === "notion_create_database") {
    return {
      title: "Notion database created",
      meta: metaOne(str(args?.title)),
      link: linkFrom(result),
    };
  }
  ...
  if (name === "github_create_issue") {
    ...
    return {
      title: n ? `Issue #${n} opened` : "Issue opened",
      meta: metaOne(str(args?.title)),
      link: linkFrom(result),
    };
  }
  if (name === "github_create_pull_request" || name === "github_create_pr") {
    ...
    return {
      title: n ? `Pull request #${n} opened` : "Pull request opened",
      meta: metaOne(str(args?.title)),
      link: linkFrom(result),
    };
  }
  ...
  if (name === "github_create_release") {
    return {
      ...
      meta: metaOne(str(args?.name) || str(args?.tag_name)),
      link: linkFrom(result),
    };
  }
  ...
  if (
    ...
    return {
      ...
      body: str(args?.content) || str(args?.message) || undefined,
    };
  }
  ...
  if (
    ...
    return {
      ...
      meta: metaOne(str(args?.to)),
      body: str(args?.message) || str(args?.text) || undefined,
    };
  }
  if (name === "telnyx_send_sms" || name === "send_sms") {
    return {
      title: "SMS sent",
      meta: metaOne(str(args?.to)),
      body: str(args?.text) || str(args?.message) || undefined,
    };
  }
  ...
  // ── Live voice session + knowledge packs (no integration brand) ──────────────
  if (name === "start_live_session") {
    const packs: Array<{ id: string; title?: string }> = Array.isArray(
      result?.attachedPacks,
    )
      ? result.attachedPacks
      : Array.isArray(args?.knowledgePackIds)
        ? args.knowledgePackIds.map((id: any) => ({ id: String(id) }))
        : [];
    const packTitles = packs
      .map((p) => str(p?.title))
      .filter(Boolean) as string[];
    const meta: React.ReactNode[] = [
      packs.length
        ? `Grounded in ${packs.length} document set${packs.length === 1 ? "" : "s"}`
        : "Answer out loud in the voice pill",
    ];
    return {
      title: "Live voice session started",
      icon: Mic,
      meta,
      chips: packTitles.length ? packTitles : undefined,
    };
  }
  if (name === "create_knowledge_pack") {
    const title = str(args?.title) || str(result?.title);
    const chunks = num(result?.chunksAdded);
    const sources = num(result?.sourcesIngested);
    const chips: string[] = [];
    if (sources && sources > 0)
      chips.push(`${sources} source${sources === 1 ? "" : "s"}`);
    if (chunks && chunks > 0)
      chips.push(`${chunks} passage${chunks === 1 ? "" : "s"}`);
    return {
...
function titleFromNotion(args: any): string | null {
  ...
  const t =
    args?.properties?.Name?.title?.[0]?.text?.content ||
...
function BrandMark({
  ...
  return (
    <span
      ...
      style={{
        background:
          "color-mix(in srgb, var(--foreground) 8%, var(--card-bg) 92%)",
        boxShadow:
    ...
    >
      {brand && (brand.logo || brand.useRemote) ? (
        ...
        <IconOverride
          style={{ width: 15, height: 15, color: "var(--foreground)" }}
          strokeWidth={2}
        />
      ) : (
        <Plug
      ...
      <span
        ...
        style={{
          background: tone === "success" ? "#22C55E" : "var(--destructive)",
      ...
      >
        {tone === "success" ? (
          ...
          <AlertTriangle
            style={{ width: 8.5, height: 8.5, color: "#fff" }}
            strokeWidth={3}
          />
        )}
      </span>
...
function Chip({ children }: { children: React.ReactNode }) {
  return (
    <span
      ...
      style={{
        background:
          "color-mix(in srgb, var(--sidebar-item-hover) 55%, transparent)",
...
function ActionCard({
  ...
  return (
    <div
      ...
      style={CARD_STYLE}
    >
      <BrandMark brand={brand} tone={tone} icon={icon} />
      <div className="min-w-0 flex-1">
        ...
        {meta && meta.length > 0 ? (
          <div className="mt-0.5 space-y-0.5">
            {meta.map((m, i) => (
              <div key={i} className="truncate text-[11px] text-theme-muted">
                {m}
              </div>
            ))}
          </div>
        ...
        {body ? (
          <div
            ...
            style={{
              background:
                "color-mix(in srgb, var(--sidebar-item-hover) 30%, transparent)",
        ...
        {link ? (
          <a
            ...
            style={{
              color:
                "color-mix(in srgb, var(--primary) 90%, var(--foreground))",
            }}
          >
...
function explainError(
  ...
  if (
    e.includes("missing_scopes") ||
    e.includes("missing user context") ||
    e.includes("not connected") ||
    e.includes("no token") ||
    e.includes("token_invalid") ||
  ...
  if (
    (e.includes("rate") && e.includes("limit")) ||
    e.includes("429") ||
    e.includes("too many requests")
  ) {
    return {
      reason: `${brand.label} is rate-limiting requests right now.`,
  ...
  if (
    ...
    return {
      reason: `${brand.label} didn’t respond in time.`,
  ...
  if (
    ...
    return {
      reason: `${brand.label} denied this action (insufficient permissions).`,
      hint: "Check the connected account’s access scopes.",
    };
  }
  if (e.includes("quota")) {
