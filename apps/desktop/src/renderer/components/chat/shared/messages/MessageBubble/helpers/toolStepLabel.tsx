...
export function getToolStepLabel(tool: ToolCall): React.ReactNode {
  ...
  switch (tool.tool) {
    ...
    case "workspace_write_file": {
      if (!filename) break;
      const verb = args.append
        ? v("Appending to", "Appended to")
        : v("Writing", "Wrote");
      return (
        <span>
          {verb}{" "}
          <InlineCodeChip title={path || undefined}>{filename}</InlineCodeChip>
        </span>
      );
    }
    case "file_edit": {
    ...
    case "workspace_read_file": {
      ...
      const range =
        Number.isFinite(ls) && Number.isFinite(le) ? ` (L${ls}–${le})` : "";
      return (
        <span>
          {v("Reading", "Read")}{" "}
          <InlineCodeChip title={path || undefined}>{filename}</InlineCodeChip>
    ...
    case "create_directory": {
      ...
      return (
        <span>
          {v("Creating folder", "Created folder")}{" "}
          <InlineCodeChip title={path || undefined}>{filename}</InlineCodeChip>
        </span>
      );
    }
    ...
    case "copy_file": {
      const src =
        typeof args.src === "string" ? getFilenameFromPath(args.src) : null;
      const dest =
        typeof args.dest === "string" ? getFilenameFromPath(args.dest) : null;
      if (!src && !dest) break;
      const verb =
        tool.tool === "copy_file"
          ? v("Copying", "Copied")
    ...
    case "analyze_media": {
      ...
      return target ? (
        <span>
          {v("Analyzing", "Analyzed")} <InlineCodeChip>{target}</InlineCodeChip>
        </span>
      ) : (
    ...
    case "web_search": {
      ...
      return q ? (
        <span>
          {v("Searching the web for", "Searched the web for")}{" "}
          <InlineCodeChip max={48}>{q}</InlineCodeChip>
        </span>
      ) : (
    ...
    case "search_local_workflows": {
      ...
      return q ? (
        <span>
          {v("Searching workflows for", "Searched workflows for")}{" "}
          <InlineCodeChip max={48}>{q}</InlineCodeChip>
        </span>
      ) : (
    ...
    case "create_knowledge_pack": {
      ...
      return title ? (
        <span>
          {v("Preparing documents", "Prepared documents")}{" "}
          <InlineCodeChip max={48}>{title}</InlineCodeChip>
        </span>
      ) : (
        v("Preparing documents", "Prepared documents")
      );
    ...
    case "query_knowledge_pack": {
      ...
      return q ? (
        <span>
          {v("Searching documents for", "Searched documents for")}{" "}
          <InlineCodeChip max={48}>{q}</InlineCodeChip>
        </span>
      ) : (
        v("Searching documents", "Searched documents")
      );
    }
    case "list_knowledge_packs":
      return v("Listing document contexts", "Listed document contexts");
    case "scrape_url": {
      const url =
      ...
      return (
        <span>
          {v("Scraping", "Scraped")}{" "}
          <InlineCodeChip title={url}>{host}</InlineCodeChip>
        </span>
      );
    }
    ...
    case "research_search": {
      const queries = Array.isArray(args.queries)
        ? args.queries.filter((q: any) => typeof q === "string" && q.trim())
        : [];
      if (queries.length === 1) {
        return (
          <span>
            {v("Researching", "Researched")}{" "}
            <InlineCodeChip max={48}>{queries[0]}</InlineCodeChip>
          </span>
        );
      }
      if (queries.length > 1) {
        return (
          <span>
            {v("Researching", "Researched")} {queries.length} angles
          </span>
        );
      }
    ...
    case "research_read": {
      ...
      return (
        <span>
          {v("Reading", "Read")}{" "}
          <InlineCodeChip title={src} max={40}>
            {label}
          </InlineCodeChip>
        </span>
      );
    ...
    case "glob": {
      ...
      return pat ? (
        <span>
          ...
          <InlineCodeChip max={48}>{pat}</InlineCodeChip>
        </span>
      ) : (
        v("Searching files", "Searched files")
      );
    ...
    case "terminal_create": {
      ...
      return cmd ? (
        <span>
          {v("Running", "Ran")} <InlineCodeChip max={64}>{cmd}</InlineCodeChip>
        </span>
      ) : (
        v("Running command", "Ran command")
      );
    }
    case "cli_agent_start": {
      ...
      if (provider && prompt) {
        return (
          <span>
            {v("Starting", "Started")} {provider}{" "}
            <InlineCodeChip max={48}>{prompt}</InlineCodeChip>
          </span>
        );
      }
      return provider
        ? `${v("Starting", "Started")} ${provider} session`
        : v("Starting CLI agent", "Started CLI agent");
    }
    case "cli_agent_send": {
      const input =
      ...
      return input ? (
        <span>
          {v("Sending to CLI", "Sent to CLI")}{" "}
          <InlineCodeChip max={56}>{input}</InlineCodeChip>
        </span>
      ) : (
    ...
    case "cli_agent_wait_for": {
      ...
      return needle ? (
        <span>
          {v("Waiting for CLI", "Waited for CLI")}{" "}
          <InlineCodeChip max={48}>{needle}</InlineCodeChip>
        </span>
      ) : (
    ...
    case "run_node_script": {
      ...
      return firstLine ? (
        <span>
          {v("Running", "Ran")} {lang}{" "}
          <InlineCodeChip max={56}>{firstLine.trim()}</InlineCodeChip>
        </span>
      ) : (
        `${v("Running", "Ran")} ${lang}`
      );
    ...
    case "ffmpeg_probe_media": {
      const input =
        typeof args.inputPath === "string"
          ? getFilenameFromPath(args.inputPath)
          : null;
      return input ? (
        <span>
          {v("Reading media info for", "Read media info for")}{" "}
          <InlineCodeChip title={args.inputPath}>{input}</InlineCodeChip>
        </span>
      ) : (
    ...
    case "ffmpeg_run": {
      const input =
        typeof args.inputPath === "string"
          ...
          : null;
      const verb =
        tool.tool === "ffmpeg_extract_audio"
          ? v("Extracting audio from", "Extracted audio from")
          : tool.tool === "ffmpeg_trim_media"
            ? v("Trimming", "Trimmed")
            : tool.tool === "ffmpeg_extract_frames"
              ? v("Extracting frames from", "Extracted frames from")
              : tool.tool === "ffmpeg_run"
                ? v("Processing", "Processed")
                : v("Converting", "Converted");
      return input ? (
        <span>
          {verb} <InlineCodeChip title={args.inputPath}>{input}</InlineCodeChip>
        </span>
      ) : (
        verb
      );
    ...
    case "browser_use_click": {
      ...
      return sel ? (
        <span>
          {v("Clicking", "Clicked")}{" "}
          <InlineCodeChip max={48}>{sel}</InlineCodeChip>
        </span>
      ) : (
        v("Clicking element", "Clicked element")
      );
    }
    case "generate_image":
    case "image_gen":
    ...
    case "edit_image": {
      ...
      const verb =
        tool.tool === "edit_image"
          ? v("Editing image", "Edited image")
          : v("Generating image", "Generated image");
      return prompt ? (
        <span>
          {verb} <InlineCodeChip max={56}>{prompt}</InlineCodeChip>
        </span>
      ) : (
        verb
      );
    ...
    case "maps_search_places": {
      const q =
        typeof args.query === "string" && args.query.trim()
          ...
          : null;
      const type =
        typeof args.included_type === "string" && args.included_type.trim()
          ? args.included_type.trim().replace(/_/g, " ")
          : null;
      const term = q || type;
      return term ? (
        <span>
          {v("Finding places", "Found places")}{" "}
          <InlineCodeChip max={56}>{term}</InlineCodeChip>
        </span>
      ) : (
    ...
    case "maps_distance_matrix": {
      const from =
        Array.isArray(args.origins) && args.origins.length
          ? String(args.origins[0])
          : null;
      const to =
        Array.isArray(args.destinations) && args.destinations.length
          ? String(args.destinations[0])
          : null;
      if (from && to) {
        return (
          <span>
            {v("Measuring distance", "Measured distance")}{" "}
            <InlineCodeChip max={32}>{from}</InlineCodeChip>{" "}
            <span className="opacity-60">→</span>{" "}
      ...
      return v(
        ...
        "Measured distance & travel time",
      );
    }
    case "maps_static_map": {
      const center =
        typeof args.center === "string" && args.center.trim()
          ? args.center.trim()
          : null;
      return center ? (
        <span>
          {v("Rendering map of", "Rendered map of")}{" "}
          <InlineCodeChip max={48}>{center}</InlineCodeChip>
        </span>
      ) : (
    ...
    case "start_live_session": {
      const packCount = Array.isArray(args.knowledgePackIds)
        ? args.knowledgePackIds.length
        : 0;
      return packCount > 0 ? (
        <span>
          {v("Starting a live voice session", "Started a live voice session")}{" "}
          grounded in {packCount}{" "}
          {packCount === 1 ? "document set" : "document sets"}
        </span>
      ) : (
        v("Starting a live voice session", "Started a live voice session")
      );
  ...
  return detail ? (
    <span>
      {name} <InlineCodeChip max={48}>{detail}</InlineCodeChip>
    </span>
  ) : (
    name
  );
