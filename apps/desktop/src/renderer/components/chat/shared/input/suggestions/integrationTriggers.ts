...
export const INTEGRATION_TRIGGERS: IntegrationTrigger[] = [
  ...
  },
  {
    ...
    verb: 'Connect',
    priority: 55,
    blurb: 'Connect your real browser so Stuard can read this page and organize your tabs.',
    keywords: ['reddit', 'comments', 'tabs'],
    phrases: [
      'this page',
      'page i am',
      'page im',
  ...
  {
    slug: 'data-analysis',
    name: getIntegrationBranding('data-analysis')?.displayName ?? 'Charts & Data',
    kind: 'install',
    verb: 'Install',
    priority: 45,
    blurb: 'Install Charts & Data to explore spreadsheets and create charts.',
    keywords: ['csv', 'dataframe', 'pandas', 'histogram', 'chart', 'plot'],
    phrases: ['analyze data', 'analyze this data', 'plot a', 'visualize the data', 'visualise the data'],
  },
  {
    slug: 'ffmpeg',
    name: getIntegrationBranding('ffmpeg')?.displayName ?? 'Media Processing',
    kind: 'install',
    verb: 'Install',
    priority: 40,
    blurb: 'Install Media Processing to convert and edit audio & video.',
    phrases: ['convert video', 'convert the video', 'trim audio', 'trim the video', 'extract audio', 'compress the video', '.mp4', '.mov'],
  },
  {
    slug: 'mediapipe',
    name: getIntegrationBranding('mediapipe')?.displayName ?? 'Vision & Motion',
    kind: 'install',
    verb: 'Install',
    priority: 38,
    blurb: 'Install Vision & Motion to detect faces, hands, and poses.',
    phrases: ['detect faces', 'face detection', 'hand tracking', 'body pose', 'recognize the image'],
  },
  {
    slug: 'ollama',
    name: getIntegrationBranding('ollama')?.displayName ?? 'Private AI',
    kind: 'install',
    verb: 'Install',
    priority: 30,
    blurb: 'Install Private AI to run models locally on your computer.',
    phrases: ['local model', 'offline ai', 'run a model locally', 'private model', 'run locally'],
  },
  {
    slug: 'python',
    name: getIntegrationBranding('python')?.displayName ?? 'Local Scripts',
    kind: 'install',
    verb: 'Install',
    priority: 28,
    blurb: 'Set up Local Scripts so Stuard can run code on your machine.',
    phrases: ['run python', 'python script', 'pip install'],
  },
  {
    slug: 'agent-cli',
    name: getIntegrationBranding('agent-cli')?.displayName ?? 'Coding Assistants',
    kind: 'install',
    verb: 'Enable',
    priority: 30,
    blurb: 'Enable Coding Assistants to delegate work to Codex, Claude Code, or Cursor.',
