...
async function fetchIntegrations(): Promise<IntegrationsResponse> {
  ...
  try {
    ...
    // ignore — endpoint will reject as unauthorized
  }
...
export function CloudVmIntegrations({
  engine,
  className,
}: {
  engine: any;
  ...
  return (
    <div className={clsx('h-full overflow-y-auto custom-scrollbar p-6 space-y-6', className)}>
      <header className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-black text-theme-fg tracking-tight">Integrations</h2>
      ...
      {/* Active deploys */}
      <section className="rounded-2xl border border-theme/10 dark:border-transparent bg-theme-card/30 p-5">
        <h3 className="text-[10px] font-black text-theme-muted uppercase tracking-wider mb-4">
          Active deploys
        </h3>
        {deploys.length === 0 ? (
          <div className="text-xs text-theme-muted">
...
function DeployStatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={clsx('px-2 py-0.5 rounded-full text-[10px] font-bold uppercase', color)}
    >
      {status}
    </span>
  );
