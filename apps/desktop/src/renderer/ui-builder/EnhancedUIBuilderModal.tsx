...
import {
  X, Eye, EyeOff, Grid3x3, ZoomIn, ZoomOut, Code2, Type, Square, MousePointer2,
...
const COMPONENT_TEMPLATES: ComponentTemplate[] = [
  ...
  { id: 'tool-call-btn', name: 'Tool Call', icon: Sparkles, category: 'workflow', html: '<button onclick="stuard.callTool(\'log\', { message: \'Hello from UI!\' }).then(r => console.log(r))" class="px-4 py-2 bg-violet-500 text-white rounded-lg font-medium hover:bg-violet-600 transition-colors flex items-center gap-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>Call Tool</button>', description: 'Calls a workflow tool via stuard.callTool()' },
];
...
// View modes
type ViewMode = 'design' | 'window' | 'flow';
...
export function EnhancedUIBuilderModal({
  ...
  // Handle HTML change from canvas
  ...
  // Track changes
  useEffect(() => {
    if (hasMountedRef.current) needsSyncRef.current = true;
  }, [html, css, js, pages, startPage, windowConfig]);
          ...
          {viewMode === 'design' && (
            <>
              ...
              <button
                ...
                className={`p-1.5 rounded-md transition-all ${previewMode ? 'bg-emerald-100 text-emerald-700' : 'text-slate-500 hover:bg-slate-100'}`}
          ...
          <div className="flex-1 flex overflow-hidden">
            <div className="flex-1 p-6 overflow-auto scrollbar-minimal bg-slate-50">
              <div className="max-w-2xl mx-auto">
                <WindowPropertiesPanel config={windowConfig} onChange={setWindowConfig} />
              </div>
            </div>
          </div>
        )}
