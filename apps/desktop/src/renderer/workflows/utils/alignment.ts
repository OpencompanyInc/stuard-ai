 ...
 * Handles cycles/loops by detecting back edges and capping recursion
 */
export interface AutoLayoutResult {
  triggers: Array<{ id: string; position: { x: number; y: number } }>;
  nodes: Array<{ id: string; position: { x: number; y: number } }>;
  backEdges: Array<{ from: string; to: string }>; // Edges that create loops
}
...
export function calculateAutoLayout(
  ...
  function detectCycles(nodeId: string) {
    if (inStack.has(nodeId)) return; // Already in current path - would be cycle
    if (visited.has(nodeId)) return; // Already fully processed
  ...
  // Find root nodes (triggers or nodes with no parents that aren't from back edges)
  const roots = allIds.filter(id => {
    const parents = parentsMap.get(id) || [];
    // Filter out parents that are from back edges
    const forwardParents = parents.filter(p => !backEdgeSet.has(`${p}|${id}`));
    return triggerIds.has(id) || forwardParents.length === 0;
  });

  // If no roots found (everything is in a cycle), use triggers or first node
  const actualRoots = roots.length > 0 ? roots : (triggers.length > 0 ? [triggers[0].id] : (allIds.length > 0 ? [allIds[0]] : []));

  // BFS to assign levels (left-to-right columns)
  // Skip back edges during level assignment to prevent infinite loops
  const queue: Array<{ id: string; level: number }> = actualRoots.map(id => ({ id, level: 0 }));
  const processed = new Set<string>();
  const MAX_LEVEL = 100; // Safety cap to prevent infinite loops
  ...
  while (queue.length > 0) {
    ...
    for (const childId of children) {
      // Skip back edges during level assignment
      if (backEdgeSet.has(`${id}|${childId}`)) continue;
