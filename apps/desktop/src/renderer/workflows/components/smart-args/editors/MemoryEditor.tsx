    ...
    <button
      ...
      className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-left transition-all ${
        checked
          ...
          : 'bg-slate-50 border border-slate-200/60 opacity-60 hover:opacity-80'
      }`}
    >
      <div
        className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
          checked ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-200 text-slate-400'
        }`}
      >
        <Icon className="w-4 h-4" />
      </div>
      <div className="flex-1 min-w-0">
        <div className={`text-sm font-medium ${checked ? 'text-slate-800' : 'text-slate-500'}`}>{meta.label}</div>
        <div className="text-xs text-slate-400 truncate">{meta.desc}</div>
      </div>
      <div
...
function ConversationHistoryEditor({
  ...
  return (
    <div className="space-y-2">
      {pairs.map((pair, pi) => (
        <div key={pi} className="relative rounded-xl border border-slate-200 bg-white overflow-hidden">
          {/* User message */}
          <div className="flex items-start gap-2 p-2.5 bg-indigo-50/40 border-b border-slate-100">
      ...
      <button
        type="button"
        onClick={addPair}
        className="flex items-center gap-2 w-full px-3 py-2.5 rounded-xl border border-dashed border-slate-300 text-slate-500 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50/30 transition-all text-sm font-medium"
      >
  ...
  return (
    <div className="space-y-2">
      ...
      <button
        type="button"
        onClick={addFact}
        className="flex items-center gap-2 w-full px-3 py-2.5 rounded-xl border border-dashed border-slate-300 text-slate-500 hover:text-amber-600 hover:border-amber-300 hover:bg-amber-50/30 transition-all text-sm font-medium"
      >
        <Plus className="w-4 h-4" />
        Add custom fact
      </button>
      ...
      {useVariableMode && (
        <div className="space-y-2">
          <p className="text-xs text-slate-400">
            Set the entire memory config from a variable (e.g. <code className="bg-slate-100 px-1 rounded">{'{{step.memory}}'}</code>) or paste raw JSON.
          </p>
          <TextInputWithVariables
            ...
            multiline
          />
          <div className="text-xs text-slate-400 bg-slate-50 rounded-lg p-2.5 border border-slate-100">
            <div className="font-medium text-slate-500 mb-1">Expected JSON shape:</div>
          ...
          >
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                config.enabled ? 'bg-indigo-500 text-white shadow-md shadow-indigo-200' : 'bg-slate-200 text-slate-400'
              }`}
            >
              <Brain className="w-5 h-5" />
            </div>
            <div className="flex-1 text-left">
              <div className={`text-sm font-semibold ${config.enabled ? 'text-indigo-700' : 'text-slate-600'}`}>
          ...
          {config.enabled && (
            <div className="space-y-2 pl-1">
              ...
              <button
                ...
                className="flex items-center gap-2 w-full px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors text-left"
              >
                {showLenses ? (
                ...
                <span className="text-sm font-medium text-slate-700">Memory Sources</span>
                <span className="text-xs text-slate-400 ml-auto">{activeLensCount}/{LENS_META.length} active</span>
              </button>
              {showLenses && (
                <div className="space-y-1.5 pl-2">
                  ...
                  <div className="px-3 py-2 flex items-center gap-3">
                    <span className="text-xs text-slate-500 shrink-0">Max related memories</span>
                    <input
                      type="range"
                      min={1}
                      max={20}
                    ...
                    />
                    <span className="text-xs font-mono text-slate-600 w-5 text-right">{config.maxFacts}</span>
                  </div>
                </div>
              )}
              ...
              >
                ...
                <span className="text-sm font-medium text-slate-700">Conversation History</span>
              ...
              {showConversation && (
                <div className="pl-2">
                  <p className="text-xs text-slate-400 mb-2 px-1">
                    ...
                    <code className="bg-slate-100 px-1 rounded">{'{{step.text}}'}</code>.
                  </p>
              ...
              >
                ...
                <span className="text-sm font-medium text-slate-700">Custom Facts</span>
              ...
              {showCustomFacts && (
                <div className="pl-2">
                  <p className="text-xs text-slate-400 mb-2 px-1">
                    ...
                    <code className="bg-slate-100 px-1 rounded">{'{{step.json}}'}</code>.
                  </p>
                  <CustomFactsEditor
