...
import {
  getSkillAgentForUser,
  clearSessionSkill,
  setSessionSkill,
} from "../../agents/skill-agent";
import { verifyAccessToken, AuthErrorCode } from "../../auth";
import {
  getOrchestratorAgentForUser,
  type BotPromptSummary,
} from "../../orchestrator";
...
import {
  queryProjectDocumentContext,
  syncProjectDocumentContext,
} from "../../tools/device/rag-packs";
...
function extractBotPromptSummaries(context: any): BotPromptSummary[] {
  const fromBotArrays = [
    ...
    Array.isArray(value)
      ? value.map((entry: any) => ({ ...entry, kind: "bot" }))
      : [],
  );
  ...
  const fromAgentArrays = [
    ...
    Array.isArray(value)
      ? value.map((entry: any) => ({ ...entry, kind: "agent" }))
      : [],
  );
    ...
    .map((path: any) => {
      ...
      const kind =
        ...
        String(path?.path || "").startsWith("agent://")
          ...
          : "bot";
      return {
        id: String(metadata.id || path.path || "").replace(
          /^(bot|agent):\/\//,
          "",
        ),
...
export async function prepareChatRequest({
  ...
  const providedMessages = Array.isArray(msg?.messages)
    ? msg.messages
    : undefined;
  ...
  const proactiveBotId = String(
    msg?.context?.proactiveBotId || msg?.context?.botId || "",
  ).trim();
  if (proactiveBotId) {
  ...
  let integrationsPromise: Promise<{
    ...
    mcpTools: {} as Record<string, any>,
    customTools: {} as Record<string, any>,
    customCatalog: [],
  });
  ...
  if (authUser) {
    ...
    const userFundedPromise = userHasUserFundedInference(
      authUser.userId,
      msg?.modelSource,
    );
    integrationsPromise = loadIntegrations(authUser.userId);
    if (needsHydration) {
      hydrationPromise = getConversationMessages(
        authUser.userId,
        hydrationConvId,
        100,
      ).catch(() => null);
    ...
    void (async () => {
      try {
        ...
        if (delivered > 0) {
          writeLog("queued_webhooks_delivered", {
    ...
    const [access, userFunded] = await Promise.all([
      accessPromise,
      userFundedPromise,
    ]);
  ...
  const routedTier = await resolveModelTier(
    msg,
    messages,
    requestedMode,
    ws,
  ...
  const modelSource =
    typeof msg?.modelSource === "string"
      ? String(msg.modelSource).trim()
      : undefined;
  console.log(
    "[cloud-ai] chat msg modelSource:",
    JSON.stringify(msg?.modelSource),
    "→ normalized:",
    modelSource ?? "(none)",
    "| keys:",
  ...
  try {
    const tierKeys =
      msg?.modelConfig && typeof msg.modelConfig === "object"
    ...
    writeLog("chat_model_resolved", {
      ...
      modelConfigTierDefault:
        typeof tierDefault === "string" ? tierDefault : null,
    });
  } catch {}
  ...
  send(
    ws,
    {
      type: "progress",
      event: "model",
      data: { tier: routedTier, modelId: chosenModelId },
    },
    requestId,
  );
  ...
  const { enabledIntegrations, mcpTools, customTools, customCatalog } =
  ...
  send(
    ws,
    { type: "progress", event: "ack", data: { ts: Date.now() } },
    requestId,
  );
  ...
  const contextPathsForMeta = Array.isArray(msg?.context?.paths)
    ? msg.context.paths
    : undefined;
  const attachmentDescriptorsForMeta = buildAttachmentMetadata(
    msg?.attachments,
  );
  ...
  if (!quickRequest) {
    const mobileSource =
      typeof msg?.mobileSource === "string"
        ? msg.mobileSource.trim().toLowerCase()
        : "";
    const ctxSource =
      typeof msg?.context?.source === "string"
        ? msg.context.source.trim().toLowerCase()
        : "";
    const phoneSignal =
      mobileSource ||
      (["sms", "mms", "whatsapp"].includes(ctxSource) ? ctxSource : "");
    if (phoneSignal) {
      promptOptions.mobileMessaging = {
        provider: phoneSignal === "whatsapp" ? "whatsapp" : "telnyx",
      };
    }
  }
  ...
  const { resource, thread } = resolveMemoryContext(
    ws,
    msg,
    authUser?.userId || "",
    conversationId,
  ...
  const resolvedModelSource =
    typeof (agent as any)?.__modelSource === "string"
...
function resolveMemoryOwner(agentType: AgentType, msg: any): MemoryOwnerScope {
  ...
  if (agentType === "bot") {
    ...
    const botId = String(
      ctx?.proactiveBotId || ctx?.botId || ctx?.bot_id || ctx?.id || "",
    ).trim();
...
async function resolveAuth(
  msg: any,
): Promise<{ authUser: AuthUser | null; authResult: any }> {
  ...
  if (!authUser) {
    const vmToken =
      typeof msg?.auth?.vmToken === "string" ? msg.auth.vmToken.trim() : "";
    const claimedUserId =
      typeof msg?.auth?.userId === "string" ? msg.auth.userId.trim() : "";
...
async function resolveModelTier(
  ...
  send(
    ws,
    {
      type: "progress",
      event: "routing",
      data: { m: routed.modelIndex, l: routed.layerIndexes },
    },
    requestId,
  );
...
async function loadIntegrations(userId: string) {
  const providers = [
    "github",
    "google",
    ...(OUTLOOK_INTEGRATION_ENABLED ? ["outlook"] : []),
    "telnyx",
  ...
  try {
    const checks = await Promise.all(
      providers.map((provider) => getExternalAccount(userId, provider)),
    );
    enabledIntegrations = providers.filter((_, index) => !!checks[index]);
  } catch {}

  try {
    const { getConnectedMCPIntegrations, getMCPToolsForIntegrations } =
      await import("../../mcp");
    const connected = await getConnectedMCPIntegrations(userId);
    if (connected.length > 0) {
      mcpTools = await getMCPToolsForIntegrations(userId, connected);
      console.log(
        `[cloud-ai] Loaded ${Object.keys(mcpTools).length} MCP tools from ${connected.length} integrations`,
      );
    }
  } catch (error) {
  ...
  try {
    const { compileInstalledToTools } =
      await import("../../integrations/compile-tools");
    ...
    if (customCatalog.length > 0) {
      console.log(
        `[cloud-ai] Loaded ${customCatalog.length} custom-integration tools`,
      );
    }
  } catch (error) {
...
function appendStoredAssistantToolCalls(history: any[], toolCalls: any) {
  const completed = (Array.isArray(toolCalls) ? toolCalls : []).filter(
    (toolCall: any) =>
      toolCall?.id && toolCall?.tool && toolCall?.result !== undefined,
  );
      ...
      content: toolResults
        ...
        .map((toolResult: any) => {
          ...
          const toolName = String(
            toolResult.toolName || toolResult.tool || toolResult.name || "tool",
          );
...
function resolveAgentType(ws: WebSocket, msg: any): AgentType {
  ...
  const clientType =
    typeof (ws as any)?.__clientType === "string"
      ? String((ws as any).__clientType)
          .toLowerCase()
          .trim()
      : "";
  const contextMode =
    typeof msg?.context?.mode === "string"
...
function resolveSkillModelId(
  ...
  return (
    process.env.SKILL_MODEL_ID ||
    process.env.WORKFLOW_MODEL_ID ||
...
async function resolveProjectPromptOptions(args: {
  ...
  const base: OrchestratorPromptOptions = {
  ...
  if (activeResearch) {
    ...
    return {
      conversationId,
      activeResearch,
      browserConnected: hasClientBridge(),
    };
  }
  ...
  try {
    ...
    const [projectResult, journalResult] = await Promise.all([
      execLocalTool("project_get", { project_id: projectId }, undefined, 5000, {
        silent: true,
      }),
      execLocalTool(
        "journal_list",
    ...
    const activeProject: ProjectContextPayload = {
      ...
      pinned_paths: Array.isArray(project.pinned_paths)
        ? project.pinned_paths
        : [],
    ...
    const recentJournal: JournalEntryPayload[] = Array.isArray(
...
async function retrieveProjectContextForQuery(args: {
  ...
  try {
    ...
    if (args.pathScopes.length > 0) {
      await syncProjectDocumentContext({
        projectId: args.projectId,
        projectName: args.projectName,
        paths: args.pathScopes,
        maxFiles: 2,
        maxChunksPerFile: 60,
      }).catch((error: any) => {
        writeLog("project_document_context_sync_failed", {
          projectId: args.projectId,
          error: error?.message || String(error),
        });
      });
    ...
    const [memoryResult, fileResult, documentResult] = await Promise.all([
      execLocalTool(
        "memory_search",
    ...
    const files = Array.isArray(fileResult?.results)
      ? fileResult.results.slice(0, 5).map((file: any) => ({
          ...
          snippet:
            file.summary ||
            (Array.isArray(file.keywords) ? file.keywords.join(", ") : null),
        }))
      : [];

    const allowedSources = new Set(
      args.pathScopes.map((p) =>
        String(p || "")
          .trim()
          .replace(/^file:\/+/, ""),
      ),
    );
    const documents = Array.isArray(documentResult?.results)
      ? documentResult.results
          ...
          .map((hit: any) => ({
            ...
            text: hit.text ?? null,
            score: typeof hit?.score === "number" ? hit.score : null,
            ordinal: typeof hit?.ordinal === "number" ? hit.ordinal : null,
          }))
      : [];

    if (memories.length === 0 && files.length === 0 && documents.length === 0)
      return null;
    return { query, memories, files, documents };
  } catch (error: any) {
...
async function resolveAgent({
  ...
  if (agentType === "workflow") {
    return await resolveWorkflowAgent(
      msg,
      providedMessages,
      workflowModelId,
      ws,
  ...
  if (agentType === "skill") {
    return await resolveSkillAgent(
      msg,
      skillModelId,
      ws,
      requestId,
  ...
  if (agentType === "bot") {
    ...
    return await getBotAgentForUser({
      botId:
        String(ctx?.proactiveBotId || ctx?.botId || "").trim() || undefined,
...
async function resolveWorkflowAgent(
  ...
  try {
    ...
    const clientType =
      typeof (ws as any)?.__clientType === "string"
        ? String((ws as any).__clientType).trim()
        : "";
    const contextMode =
      typeof msg?.context?.mode === "string"
        ? String(msg.context.mode).trim()
        : "";
    ...
    if (
      directWorkflow &&
      typeof directWorkflow === "object" &&
      !Array.isArray(directWorkflow)
    ) {
    ...
    if (workspacePath) {
      ...
      const flowId =
        ...
        directWorkflow.id
          ...
          : typeof incomingContext?.workflowId === "string"
            ? incomingContext.workflowId
            : "";
      if (flowId) setWorkflowWorkspacePath(flowId, String(workspacePath));
    }

    if (!sessionLoaded) {
      const workflowMessage = providedMessages?.find(
        (message: any) =>
          message?.role === "system" &&
      ...
      if (workflowMessage && typeof workflowMessage.content === "string") {
        ...
        if (
          workflowJson &&
          (workflowJson.id || workflowJson.triggers || workflowJson.nodes)
        ) {
    ...
    const nodeCount = Array.isArray(loadedWorkflow?.nodes)
      ? loadedWorkflow.nodes.length
      : 0;
    ...
    if (editMode && loadedWorkflow) {
      try {
        ...
        (agent as any).__workflowDsl = serializeWorkflow(loadedWorkflow, {
          mode,
          abbreviateOver: 600,
        });
      } catch (err: any) {
        console.warn(
          "[cloud-ai] workflow DSL context serialize failed:",
          err?.message,
        );
      }
    }
    ...
    send(
      ...
      {
        type: "error",
        message: "Workflow agent unavailable: " + error.message,
      },
      requestId,
    );
    return null;
  ...
  try {
    ...
    const contextMode =
      typeof msg?.context?.mode === "string"
        ? String(msg.context.mode).trim()
        : "";
    console.log("[cloud-ai] Using skill agent", {
      rawAgent,
      clientType,
      ctxMode: contextMode,
    ...
    if (
      incomingSkill &&
      typeof incomingSkill === "object" &&
      !Array.isArray(incomingSkill)
    ) {
    ...
    send(
      ws,
      { type: "error", message: "Skill agent unavailable: " + error.message },
      requestId,
    );
    return null;
  }
...
function buildAttachmentMetadata(rawAttachments: any): any[] | undefined {
  ...
  const serialized = attachments
    ...
    .map((attachment: any) => ({
      ...
      source:
        typeof attachment.source === "string" ? attachment.source : undefined,
      previewText:
        typeof attachment.previewText === "string"
          ? attachment.previewText.slice(0, 600)
          : undefined,
      lineCount:
...
function resolveMemoryContext(
  ...
  if (
    typeof incomingMemory?.resource === "string" &&
    incomingMemory.resource.trim()
  ) {
    resource = incomingMemory.resource.trim();
  }
  if (
    typeof incomingMemory?.thread === "string" &&
    incomingMemory.thread.trim()
  ) {
