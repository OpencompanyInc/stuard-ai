...
function makeAskOrchestratorTool(correlation: SubagentCorrelation) {
  return createTool({
    id: 'ask_orchestrator',
    description:
    ...
    execute: async ({ question, choices }) => {
      ...
      const answerPromise = new Promise<string>((resolve, reject) => {
        ...
        setTimeout(() => {
          if (pendingQuestions.has(questionId)) {
            pendingQuestions.delete(questionId);
            reject(new Error('Orchestrator did not answer within timeout'));
          }
        }, 120_000);
      });

      // Emit the question as a tool result the orchestrator can see
      // The orchestrator's delegation tool will intercept this via the
      // event callback and surface it to the parent agent.
      ...
      // Store on the correlation for the outer runner to pick up
      (correlation as any).__pendingQuestion = questionMsg;
...
function buildSubagent(
  ...
  const askTool = makeAskOrchestratorTool(correlation);
...
export async function runSubagent(opts: RunSubagentOptions): Promise<DelegationResult> {
  ...
  onEvent?.({
    ...
    runId,
    event: 'started',
    data: { kind: request.kind, label: request.kind },
  });
  ...
  const agent = buildSubagent(pack, correlation, model, modelId, bridgeWs, bridgeSecrets);
  ...
  try {
    ...
    // Set up question interception
    const originalAskExecute = (agent as any).tools?.ask_orchestrator?.execute;
    if (onQuestion && originalAskExecute) {
      const wrappedAsk = createTool({
        id: 'ask_orchestrator',
        ...
        execute: async ({ question, choices }) => {
          ...
          writeLog('subagent_ask_forwarded', { subagentId, questionId, question });
      ...
      (agent as any).tools.ask_orchestrator = wrappedAsk;
    }
    ...
    while (attempt <= MAX_RETRIES) {
      ...
      try {
        ...
        console.log(`[subagent:${subagentId}] completed in ${durationMs}ms | text=${text.length}chars | steps=${steps.length} | toolCalls=${toolCalls.length}`);
        ...
        writeLog('subagent_complete', { subagentId, ok: true, durationMs, textLength: text.length, toolCallCount: toolCalls.length, stepsCount: steps.length });
