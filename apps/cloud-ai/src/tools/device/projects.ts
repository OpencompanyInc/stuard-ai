...
function dirnameForLocalPath(raw: string): string | null {
  ...
  if (!p) return null;
  const parser =
    /^[a-zA-Z]:[\\/]/.test(p) || p.includes("\\") ? path.win32 : path;
  const dir = parser.dirname(p);
  return dir && dir !== "." && dir !== p ? dir : null;
}

async function readPinnedFileAsMemory(
  projectId: string,
  filePath: string,
): Promise<any | null> {
  try {
...
async function attachProjectContextPath(
  projectId: string,
  rawPath: string,
  options: { processLimit?: number; scan?: boolean; writer?: any } = {},
): Promise<any> {
  ...
  const projectResult = await execLocalTool("project_get", {
  ...
  const updated = alreadyPinned
    ? projectResult
    : await execLocalTool("project_update", {
        project_id: projectId,
        pinned_paths: next,
      });
  ...
  const addRoot = await execLocalTool(
    "file_index_add_root",
    {
      path: cleanPath,
      schedule: "daily",
  ...
  if (addRoot?.ok && addRoot.root) {
    ...
    if (options.scan !== false) {
      ...
      try {
        embeddingProgress = await fileIndexingService.processPendingFiles(
          options.processLimit ?? 75,
        );
      } catch (error) {
    ...
    documentContext = await ingestProjectContextPath({
      projectId,
    ...
    if (parent) {
      const parentRoot = await execLocalTool(
        "file_index_add_root",
        {
          path: parent,
          schedule: "daily",
...
export const list_projects = createTool({
  ...
  execute: async (inputData) => {
    ...
    if (!hasClientBridge())
      return { ok: false, error: ERR_NO_BRIDGE, projects: [] };
...
export const create_project = createTool({
  ...
  inputSchema: z.object({
    ...
    description: z
      .string()
      .optional()
      .describe("One-line description of what the project is."),
    goals: z
      .string()
      .optional()
    ...
    instructions: z
      ...
      .optional()
      .describe(
        "Standing project instructions applied to every chat in this project, similar to Claude Project instructions or Perplexity Space answer instructions.",
      ),
    status: z
    ...
    color: z
      ...
      .optional()
      .describe(
        'Hex color like "#71717a". Omit to use the default neutral; only set if the user requested a specific color or one strongly matches the project.',
      ),
  ...
  execute: async (inputData) => {
    ...
    const embedText = [c.name, c.description, c.goals, c.instructions]
      .filter(Boolean)
      .join("\n\n");
...
export const update_project = createTool({
  ...
  inputSchema: z.object({
    ...
    pinned_paths: z
      ...
      .optional()
      .describe(
        "Full replacement list of pinned file paths shown in the project's Files tab. Pass an empty array to clear. For single-file edits prefer pin_file/unpin_file.",
      ),
  }),
  outputSchema: z.object({
    ok: z.boolean(),
...
export const pin_file = createTool({
  ...
  inputSchema: z.object({
    ...
    path: z
      .string()
      .describe("Absolute file or folder path to attach as project context."),
  }),
  outputSchema: z.object({
    ok: z.boolean(),
    project: z.any().optional(),
  ...
  execute: async (inputData, runCtx) => {
    ...
    const attached = await attachProjectContextPath(c.project_id, c.path, {
      processLimit: 25,
      writer,
    });
    return {
...
export const add_project_context = createTool({
  ...
  inputSchema: z.object({
    ...
    paths: z
      .array(z.string())
      .min(1)
      .describe("Absolute file/folder paths to add as project context."),
  ...
  execute: async (inputData, runCtx) => {
    ...
    const writer = (runCtx as any)?.writer;
    if (!hasClientBridge())
      return { ok: false, error: ERR_NO_BRIDGE, results: [] };
    const paths: string[] = Array.isArray(c.paths) ? c.paths : [];
    const perPathLimit = Math.max(
      0,
      Math.floor((c.process_limit ?? 150) / Math.max(paths.length, 1)),
    );
    const results = [];
    for (const p of paths) {
      results.push(
        await attachProjectContextPath(c.project_id, p, {
          processLimit: perPathLimit,
          writer,
        }),
      );
    }
...
export const unpin_file = createTool({
  ...
  inputSchema: z.object({
    ...
    path: z
      .string()
      .describe(
        "Absolute file path to unpin. Must match the pinned path exactly.",
      ),
  }),
  outputSchema: z.object({
    ok: z.boolean(),
  ...
  execute: async (inputData) => {
    ...
    if (!current.includes(path)) {
      return {
        ...
        pinned_paths: current,
        was_pinned: false,
      };
    }
    const next = current.filter((p) => p !== path);
    const updated = await execLocalTool("project_update", {
      project_id: c.project_id,
      pinned_paths: next,
    });
...
export const enter_project_mode = createTool({
  ...
  inputSchema: z.object({
    ...
    project_id: z
      .string()
      .describe("The project to enter (from list_projects)."),
  }),
  outputSchema: z.object({
    ok: z.boolean(),
    project: z.any().optional(),
  ...
  execute: async (inputData) => {
    ...
    if (!setResult?.ok)
      return { ok: false, error: setResult?.error || "failed_to_set_project" };

    // Fetch project + recent journal so the AI's next turn can warm-welcome.
    const projectResult = await execLocalTool("project_get", {
      project_id: c.project_id,
    });
    const journalResult = await execLocalTool("journal_list", {
      project_id: c.project_id,
      limit: 5,
    });
...
export const exit_project_mode = createTool({
  ...
  execute: async (inputData) => {
    ...
    if (!setResult?.ok)
      return {
        ok: false,
        error: setResult?.error || "failed_to_clear_project",
      };
...
export const journal_add = createTool({
  ...
  inputSchema: z.object({
    ...
    type: z
      ...
      .default("finding")
      .describe(
        "Pick the most specific type. Avoid `milestone` unless work actually shipped.",
      ),
    ...
    source_ref: z
      ...
      .optional()
      .describe(
        "Optional links: { conversation_id?, commit_sha?, file_paths?, task_id? }",
      ),
  }),
  outputSchema: z.object({
    ok: z.boolean(),
...
export const memory_add = createTool({
  ...
  inputSchema: z.object({
    content: z
      .string()
      .describe("The memory body — the fact, note, or snippet itself."),
    title: z
      .string()
      .optional()
      .describe("Short headline. Optional but improves recall."),
    type: z
      .enum(["note", "fact", "snippet", "link", "file", "image"])
      .optional()
      .default("note")
      .describe(
        'Memory kind. Most things are "note" or "fact"; use "snippet" for code, "link" for URLs.',
      ),
    project_ids: z
      .array(z.string())
      .optional()
      .describe(
        "Project IDs to scope this memory to. Omit while in project mode to default to the active project. Pass `[]` explicitly to save globally.",
      ),
    conversation_id: z
      .string()
      .optional()
      .describe(
        "Pass when in project mode (from <conversation> in your system prompt) so the active project can be resolved automatically.",
      ),
  ...
  execute: async (inputData) => {
    ...
    let projectIds: string[] | undefined = Array.isArray(c.project_ids)
      ? c.project_ids
      : undefined;
...
export const project_search = createTool({
  ...
  outputSchema: z.object({
    ...
    document_results: z.array(z.any()).optional(),
    count: z.number().optional(),
    error: z.string().optional(),
  }),
  execute: async (inputData) => {
    ...
    let documentResult: any = { ok: true, results: [] };
    let pathScopes: string[] = [];
    if (c.include_files !== false) {
      ...
      pathScopes = Array.isArray(projectResult?.project?.pinned_paths)
        ? projectResult.project.pinned_paths
        : [];
      ...
      if (pathScopes.length > 0) {
        ...
        documentResult = await queryProjectDocumentContext({
    ...
    const pinnedSources = new Set(pathScopes.map(normalizeProjectPath));

    const memoryResults = Array.isArray(memoryResult?.results)
      ? memoryResult.results.map((result: any) => ({
          ...result,
          source_type: "memory",
        }))
      : [];
    const fileResults = Array.isArray(fileResult?.results)
      ...
      : [];
    const documentResults = Array.isArray(documentResult?.results)
      ? documentResult.results
          .filter(
            (result: any) =>
              pinnedSources.size === 0 ||
              pinnedSources.has(normalizeProjectPath(result?.source)),
          )
          .map((result: any) => ({
            document: result,
            score: Number(result?.score || 0),
      ...
      : [];
    const results = [...memoryResults, ...documentResults, ...fileResults]
    ...
    return {
      ok:
        ...
        fileResult?.ok !== false &&
        documentResult?.ok !== false,
      results,
      memory_results: memoryResults,
      file_results: fileResults,
      document_results: documentResults,
      count: results.length,
      error: memoryResult?.error || fileResult?.error || documentResult?.error,
    };
  },
});
...
export const search_project_conversations = createTool({
  ...
  inputSchema: z.object({
    conversation_id: z
      ...
      .optional()
      .describe(
        "Current conversation ID. Used to resolve the active project. Required unless project_id is passed.",
      ),
    project_id: z
      .string()
      .optional()
      .describe(
        "Explicit project to scope to. If omitted, resolved from conversation_id.",
      ),
    query: z.string().describe("Natural-language search query."),
  ...
  execute: async (inputData) => {
    ...
    if (!projectId) {
      return {
        ok: false,
        error:
          "no_active_project — pass project_id explicitly, or enter Project Mode first.",
        results: [],
      };
    }
