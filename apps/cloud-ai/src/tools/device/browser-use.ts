...
export const browser_use_tabs = makeLocalTool(
  ...
  z.object({
    action: z.enum(['list', 'new', 'switch', 'close', 'release']).describe('Tab action'),
    ...
    session_id: z.string().optional().describe('Internal browser session id for tab ownership'),
    tab_index: z.number().optional().describe('Internal assigned tab index for this browser session'),
  }),
  z.object({
    ok: z.boolean(),
