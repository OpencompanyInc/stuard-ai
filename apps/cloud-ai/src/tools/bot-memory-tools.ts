...
/**
 * Agent's private kanban + run-log tools.
 *
 ...
 * the user's task board. Treat `agent_memory_*` as your own working memory:
...
export const agent_memory_create = makeLocalTool(
  'agent_memory_create',
  'Add a card to YOUR private kanban. Use this to capture an intent ("queued"), something you are actively doing ("in_progress"), or a finished outcome ("completed"). Notes can hold context your future self should see.',
  z.object({
    title: z.string().min(1).describe('Short, scannable title (one line).'),
    notes: z.string().optional().describe('Longer context - what you tried, what you decided, links, etc. Optional.'),
    status: cardStatusEnum.optional().default('queued').describe('Defaults to "queued".'),
...
export const agent_memory_update = makeLocalTool(
  'agent_memory_update',
  'Update one of YOUR private kanban cards - change its status, edit its title, or replace notes. Use this when you finish something, decide to retry, or learn something new.',
  z.object({
    id: z.string().min(1).describe('The card id from agent_memory_list.'),
    title: z.string().optional(),
    notes: z.string().optional().describe('Replaces existing notes. Pre-pend the existing notes if you want to append.'),
    status: cardStatusEnum.optional(),
  }),
...
export const agent_memory_delete = makeLocalTool(
  'agent_memory_delete',
  'Remove a card from YOUR private kanban. Use sparingly - usually mark cards "completed" instead so the history is preserved.',
  z.object({
    id: z.string().min(1),
...
export const agent_memory_log = makeLocalTool(
  'agent_memory_log',
  'Append an entry to YOUR private run log so the next run can see what just happened. The scheduler auto-logs a brief outcome line - call this only when you want a richer entry. One call per run is plenty.',
  z.object({
    summary: z.string().min(1).describe('One-line "what happened this run" - keep it short and useful.'),
    outcome: z.enum(['success', 'partial', 'failed']).optional().default('success'),
    cardIds: z.array(z.string()).optional().describe('Card ids touched this run.'),
