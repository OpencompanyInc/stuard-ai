...
export async function handleGithubRoutes(req: IncomingMessage, res: ServerResponse, parsedUrl: URL): Promise<boolean> {
  ...
  if (req.method === 'GET' && parsedUrl.pathname === GITHUB_REDIRECT_PATH) {
    try {
      ...
      try { await upsertExternalAccount({ userId, provider: 'github', access_token, scopes, refresh_token: null, expires_at: null, meta: { token_type: tokenBody.token_type || 'bearer' }, profileLabel, accountEmail }); } catch {}
      let okSaved = false;
      try { const acc = await getExternalAccount(userId, 'github', profileLabel); okSaved = !!acc; } catch { okSaved = false; }
      if (!okSaved) {
