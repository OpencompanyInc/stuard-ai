...
_TOOL_METADATA: Dict[str, tuple[str, str]] = {
    ...
    "computer_use": (
        "input",
        "Perform GUI actions (mouse/keyboard) and optionally capture a screenshot",
    ),
    ...
    "scroll": (
        "input",
        "Scroll the mouse wheel vertically and optionally horizontally",
    ),
    ...
    "glob": (
        "system",
        "Find files by glob pattern (e.g. **/*.pdf). Requires root for **; **/* is rejected.",
    ),
    "grep": (
        "system",
        "Search text in files (regex or literal). Supports searching inside PDFs, XLSX, and XLS by extracting document text first.",
    ),
    ...
    "python_list_packages": (
        "system",
        "List installed Python packages in a managed venv",
    ),
    ...
    "data_analysis_status": (
        "data_analysis",
        "Check data analysis env + required-package install status",
    ),
    "data_analysis_setup": (
        "data_analysis",
        "Create venv and install pandas/numpy/scipy/matplotlib/seaborn/openpyxl",
    ),
    "data_analysis_uninstall": (
        "data_analysis",
        "Remove the data analysis env and free disk space",
    ),
    "data_load": (
        "data_analysis",
        "Peek at a CSV/XLSX/JSON/Parquet file: columns, dtypes, shape, sample rows",
    ),
    "describe_data": (
        "data_analysis",
        "Pandas describe()-style summary stats for numeric columns",
    ),
    "correlate_data": (
        "data_analysis",
        "Correlation matrix (Pearson/Spearman/Kendall) for numeric columns",
    ),
    "plot_line": (
        "data_analysis",
    ...
    "plot_scatter": (
        "data_analysis",
        "Render a scatter plot with optional regression line to PNG",
    ),
    "plot_hist": (
        "data_analysis",
        "Render a histogram (with optional KDE overlay) to PNG",
    ),
    "plot_pie": ("data_analysis", "Render a pie or donut chart to PNG"),
    "plot_heatmap": (
        "data_analysis",
        "Render a seaborn heatmap from a 2D matrix to PNG",
    ),
    "plot_box": ("data_analysis", "Render a box plot (single or grouped) to PNG"),
    "run_data_python": (
        "data_analysis",
        "Run arbitrary Python with pandas/numpy/scipy/matplotlib/seaborn pre-loaded",
    ),
    # Desktop controls
    "describe_desktop_control_capabilities": (
        "desktop",
        "Describe available desktop software-control backends",
    ),
    "get_desktop_wallpaper": ("desktop", "Get the current desktop wallpaper path"),
    "set_desktop_wallpaper": (
        "desktop",
        "Set the desktop wallpaper from a local image path",
    ),
    "get_system_volume": ("desktop", "Get current system output volume and mute state"),
    "set_system_volume": (
        "desktop",
        "Set or adjust system output volume and mute state",
    ),
    "list_bluetooth_devices": ("desktop", "List known Bluetooth devices"),
    "connect_bluetooth_device": (
        "desktop",
        "Connect a Bluetooth device when the platform backend supports it",
    ),
    "disconnect_bluetooth_device": (
        "desktop",
        "Disconnect a Bluetooth device when the platform backend supports it",
    ),
    "get_display_brightness": (
        "desktop",
        "Get laptop or display brightness when available",
    ),
    "set_display_brightness": (
        "desktop",
        "Set laptop or display brightness when available",
    ),
    ...
    "knowledge_get_facts_for_conversation": (
        "knowledge",
        "Get facts extracted from a conversation",
    ),
    "knowledge_get_conversations_for_entity": (
        "knowledge",
    ...
    "rag_pack_create": ("rag", "Create a new (empty) knowledge pack / RAG namespace"),
    "rag_pack_list": ("rag", "List knowledge packs"),
    "rag_pack_get": ("rag", "Get a knowledge pack and its stats by id"),
    "rag_pack_delete": ("rag", "Delete a knowledge pack and all its chunks"),
    "rag_pack_delete_source": ("rag", "Delete one source from a knowledge pack"),
    "rag_pack_add_chunks": ("rag", "Add pre-embedded chunks to a knowledge pack"),
    "rag_pack_query": ("rag", "Search a single knowledge pack by query vector"),
    "rag_pack_stats": ("rag", "Get chunk/source counts for a knowledge pack"),
    "rag_project_pack_get_or_create": (
        "rag",
        "Get/create hidden project document context",
    ),
    "rag_project_pack_stats": ("rag", "Get hidden project document context stats"),
    "rag_project_pack_query": ("rag", "Search hidden project document context"),
    "rag_project_pack_delete": ("rag", "Delete hidden project document context"),
    # Pending Memories
    ...
    "pending_memory_expire": (
        "memory",
        "B4: expire pending memories past TTL and cap active count",
    ),
    "knowledge_consolidate_facts": (
        "knowledge",
        "B2: pairwise vector dedup for a (category, subtype) slice",
    ),
    # Memory (Conversations & Projects)
    ...
    "conversation_get_extraction_offset": (
        "memory",
        "B1: highest turn index already extracted for a conversation",
    ),
    "conversation_set_extraction_offset": (
        "memory",
        "B1: advance extraction watermark for a conversation",
    ),
    ...
    "segment_build_topic_drawers": (
        "memory",
        "Build topic drawers (topics -> clustered segments)",
    ),
    ...
    "project_context_add": (
        "data",
        "Attach a file or folder as indexed project context",
    ),
    ...
    "journal_get": ("data", "Get a journal entry by ID"),
    "journal_update": ("data", "Update a journal entry in place"),
    "journal_list": ("data", "List journal entries for a project"),
    "journal_delete": ("data", "Delete a journal entry"),
    "conversation_set_project": (
        "data",
        "Stamp a conversation with a project_id (or clear it)",
    ),
    ...
    "memory_export_plaintext": (
        "memory",
        "Export memory.db with all encrypted columns decrypted (for VM sync)",
    ),
    # Media capture
    "capture_media": ("vision", "Capture photos, videos, or audio"),
    "stop_capture": ("vision", "Stop an active capture session"),
    "stop_captures_by_flow": (
        "vision",
        "Stop capture sessions belonging to a specific workflow (by flowId)",
    ),
    "list_active_captures": ("vision", "List active capture sessions"),
    "describe_media_capture_capabilities": (
        "vision",
        "Describe media capture capabilities",
    ),
    ...
    "describe_screen_capture_capabilities": (
        "vision",
        "Describe screen capture capabilities",
    ),
    ...
    "describe_system_audio_capabilities": (
        "vision",
        "Describe system audio capabilities",
    ),
    # MediaPipe
    ...
    "mediapipe_face_detection": (
        "vision",
        "Detect faces with bounding boxes and keypoints using MediaPipe",
    ),
    "mediapipe_face_mesh": ("vision", "Detect 468 face mesh landmarks using MediaPipe"),
    "mediapipe_segmentation": (
        "vision",
        "Segment person from background (selfie segmentation) using MediaPipe",
    ),
    "mediapipe_holistic": (
        "vision",
        "Detect pose + hands + face in one pass using MediaPipe Holistic",
    ),
    "mediapipe_process_video": (
        "vision",
        "Process video frames with MediaPipe (pose/hands/face/holistic)",
    ),
    # FFmpeg
    ...
    "unified_task_assignments": (
        "integrations",
        "Manage user task assignments (reminders, actions, check-ins scheduled for the agent)",
    ),
    ...
    "vault_get_credential": (
        "vault",
        "Get credential (username/password) for agent use",
    ),
    ...
    "folder_permission_add": (
        "system",
        "Add a folder to the allowed access list with read/write/both permission",
    ),
    "folder_permission_remove": (
        "system",
        "Remove a folder from the allowed access list",
    ),
    ...
    "db_query": (
        "data",
        "Execute raw SQL against the local workflow database (SQLite)",
    ),
    ...
    "http_request": (
        "integrations",
        "Make HTTP requests like curl or Postman (GET, POST, PUT, PATCH, DELETE, etc.)",
    ),
    # Streaming
    "stream_create": (
        "streaming",
        "Create a named data stream for real-time chunk processing",
    ),
    ...
    "close_all_streams": (
        "streaming",
        "Close ALL active streams, optionally filtered by flowId",
    ),
    "stream_get_status": ("streaming", "Get detailed stream stats and subscriber info"),
    "stream_from_script": (
        "streaming",
        "Run a Python script that emits chunks into a real-time stream",
    ),
    "stream_from_api": (
        "streaming",
        "Subscribe to a streaming API (SSE/chunked HTTP) and push events into a stream",
    ),
    "stream_from_llm": (
        "streaming",
        "Stream LLM text generation token-by-token into a workflow stream",
    ),
}
...
_HANDLERS = {
    ...
    # Knowledge Packs (sandboxed RAG namespaces)
    "rag_pack_create": rag.rag_pack_create,
    "rag_pack_list": rag.rag_pack_list,
    "rag_pack_get": rag.rag_pack_get,
    "rag_pack_delete": rag.rag_pack_delete,
    "rag_pack_delete_source": rag.rag_pack_delete_source,
    "rag_pack_add_chunks": rag.rag_pack_add_chunks,
    "rag_pack_query": rag.rag_pack_query,
    "rag_pack_stats": rag.rag_pack_stats,
    "rag_project_pack_get_or_create": rag.rag_project_pack_get_or_create,
    "rag_project_pack_stats": rag.rag_project_pack_stats,
    "rag_project_pack_query": rag.rag_project_pack_query,
    "rag_project_pack_delete": rag.rag_project_pack_delete,
    # Memory (Conversations & Projects)
    ...
    "journal_get": memory_conversations.journal_get,
    "journal_update": memory_conversations.journal_update,
...
_TOOL_METADATA["list_tools"] = (
    "core",
    "List all available tools with optional category filter",
)
_TOOL_METADATA["get_tool_info"] = (
    "core",
    "Get detailed information about a specific tool",
)
_TOOL_METADATA["list_tool_categories"] = (
    "core",
...
def get_all_categories() -> Dict[str, Any]:
    ...
    categories = [
        {"id": cat, "count": count} for cat, count in sorted(category_counts.items())
    ]
