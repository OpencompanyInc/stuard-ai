...
export function KanbanTab({
  ...
  const handleMove = async (card: KanbanCard, status: KanbanStatus) => {
    ...
    await platform.memoryUpdateCard?.(botId, card.id, { status });
    onChanged();
  };
  ...
  const handleSave = async (patch: { title: string; notes?: string; status: KanbanStatus }) => {
    if (editingCard) {
      await platform.memoryUpdateCard?.(botId, editingCard.id, patch);
    } else if (creatingInColumn) {
      await platform.memoryCreateCard?.(botId, { ...patch, status: creatingInColumn });
    }
    setEditingCard(null);
...
function KanbanCardItem({
  ...
  return (
    <div
      onClick={onClick}
      className={clsx(
        'group relative cursor-pointer rounded-xl border border-[color:var(--dashboard-panel-border)] bg-zinc-500/10 p-3 shadow-sm transition hover:bg-theme-hover/25 hover:ring-2',
        column.ringTone,
      )}
    >
      ...
      {menuOpen && (
        <div
          className="absolute right-2 top-9 z-10 w-44 rounded-lg border border-[color:var(--dashboard-panel-border)] bg-theme-card p-1 shadow-lg"
          ...
          {runLog.map(entry => {
            ...
            return (
              <li key={entry.id} className="relative">
                ...
                <div className="rounded-xl border border-[color:var(--dashboard-panel-border)] bg-zinc-500/10 px-3 py-2 shadow-sm">
        ...
        <div className="space-y-4 p-5">
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-[color:var(--dashboard-panel-border)] bg-theme-card/60 px-3 py-2.5 text-[14px] text-theme-fg outline-none transition focus:border-primary/60"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full resize-none rounded-xl border border-[color:var(--dashboard-panel-border)] bg-theme-card/60 px-3 py-2.5 text-[13px] leading-6 text-theme-fg outline-none transition focus:border-primary/60"
            />
          </div>
                  ...
                  <button
                    ...
                    className={clsx(
                      ...
                      active
                        ...
                        : 'border-theme/30 bg-theme-card/40 text-theme-muted hover:bg-theme-hover/40',
                    )}
                  >
